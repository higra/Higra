...
namespace xsimd {
    ...
    inline batch<float, 4> sqrt(const batch<float, 4> &lhs) {
        ...
        batch<float, 4> sqrt_approx =
                lhs * sqrt_reciprocal * batch<float, 4>(vrsqrtsq_f32(lhs * sqrt_reciprocal, sqrt_reciprocal));
    ...
    inline batch<float, 4>
    select(const batch_bool<float, 4> &cond, const batch<float, 4> &a, const batch<float, 4> &b) {
