...
#  pragma GCC diagnostic pop
        ...
        void initialize_generic(detail::function_record *rec, const char *text,
            ...
            if (rec->is_constructor && !rec->is_new_style_constructor) {
                const auto class_name = std::string(((PyTypeObject * )
                rec->scope.ptr())->tp_name);
        ...
        static PyObject *dispatcher(PyObject *self, PyObject *args_in, PyObject *kwargs_in) {
            ...
            if (overloads->is_constructor) {
                const auto tinfo = get_type_info((PyTypeObject * )
                overloads->scope.ptr());
...
/// Generic support for creating new Python heap types
        ...
        protected:
            void initialize(const type_record &rec) {
                ...
                internals.registered_types_py[(PyTypeObject * )
                m_ptr] = { tinfo };
                ...
                if (rec.bases.size() > 1 || rec.multiple_inheritance) {
                    ...
                    auto parent_tinfo = get_type_info((PyTypeObject * )
                    rec.bases[0].ptr());
            ...
            void mark_parents_nonsimple(PyTypeObject *value) {
                ...
                for (handle h : t) {
                    auto tinfo2 = get_type_info((PyTypeObject * )
                    h.ptr());
                    if (tinfo2)
                        tinfo2->simple_type = false;
                    mark_parents_nonsimple((PyTypeObject * )
                    h.ptr());
                }
            }
...
/// Binds C++ enumerations and enumeration classes to Python
    ...
    NAMESPACE_BEGIN(detail)
        ...
        inline std::pair<
        decltype(internals::registered_types_py)::iterator,
        bool>
        all_type_info_get_cache(PyTypeObject
        *type) {
