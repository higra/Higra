...
@hg.dataConsumer("attr0")
def consumer0(obj, attr0):
    return attr0


@hg.dataConsumer("attr1")
def consumer1(obj, attr1):
    return attr1


@hg.dataConsumer(attr1="attr1")
def consumer2(obj, attr1):
    return attr1


@hg.dataConsumer("attr1", attr2="dep.attr2")
def consumer3(obj, attr1, attr2):
    return attr1 + attr2


@hg.dataProvider("attr2")
@hg.dataConsumer("attr1")
def procon(obj, attr1, crash=False):
    if crash:
        raise Exception("Should not have been called")
    print(attr1)
    return attr1 + 1
...
class TestDataCache(unittest.TestCase):
    ...
    def test_provider(self):
        ...
        self.assertRaises(Exception, provider1, obj1, True, forceRecompute=True)
        hg.clearAllAttributes()
    ...
    def test_consumer(self):
        ...
        hg.setAttribute(obj3, "attr0", 1)
        self.assertTrue(consumer0(obj3) == 1)
        hg.clearAllAttributes()

        obj4 = Dummy(4)
        self.assertTrue(consumer1(obj4) == 1)
        self.assertTrue(hg.getAttribute(obj4, "attr1") == 1)
        hg.clearAllAttributes()

        obj5 = Dummy(5)
        self.assertTrue(consumer2(obj5) == 1)
        self.assertTrue(hg.getAttribute(obj5, "attr1") == 1)
        hg.clearAllAttributes()

    def test_providerconsumer(self):
        obj2 = Dummy(2)
        self.assertTrue(procon(obj2, crash=False) == 2)
        self.assertTrue(procon(obj2, crash=True) == 2)
        self.assertRaises(Exception, procon, obj2, crash=True, forceRecompute=True)
        hg.clearAllAttributes()
    ...
    def test_consumer_repath(self):
        ...
        self.assertTrue(hg.getAttribute(obj6, "attr2") == 2)
        hg.clearAllAttributes()
    ...
    def test_consumer_compoundPath(self):
        ...
        hg.setAttribute(obj7, "dep", obj8)
        self.assertTrue(consumer3(obj7) == 3)
        self.assertTrue(hg.getAttribute(obj8, "attr2") == 2)
        self.assertTrue(hg.getAttribute(obj8, "attr1") == 1)
        hg.clearAllAttributes()
