...
void py_init_contour_segment_2d(pybind11::module &m) {
    ...
    auto c = py::class_<class_t>(m, "ContourSegment2d",
                                 "A contour segment is an ordered list of contour elements that "
    ...
    c.def("distance_to_point", &class_t::distance_to_point,
          "Distance between the given point and the line "
...
void py_init_polyline_contour_2d(pybind11::module &m) {
    ...
    auto c = py::class_<class_t>(m, "PolylineContour2d",
                                 "A polyline contour is an ordered list of contour segment "
                                 "that form a continuous piece of frontier between 2 regions.\n"
                                 "Note that consecutive segments share their extremities: given two consecutive "
                                 "contour segments s1 and s2, "
    ...
    c.def("subdivide", &class_t::subdivide,
...
void py_init_contour_2d_impl(pybind11::module &m) {
    ...
    c.def("__getitem__", [](class_t &c, index_t i) { return c[i]; });
    c.def("subdivide", &class_t::subdivide,
          "Subdivide each segment of the contour such that: "
          "For each segment, the distance between the line joining the extremities "
          "of the segment and each of its elements is lower than the given threshold ("
          "Ramer–Douglas–Peucker algorithm) or smaller than the minimal specified size"
          "\n"
          "The threshold is equal to\n"
          "- epsilon if relative_epsilon is false\n"
